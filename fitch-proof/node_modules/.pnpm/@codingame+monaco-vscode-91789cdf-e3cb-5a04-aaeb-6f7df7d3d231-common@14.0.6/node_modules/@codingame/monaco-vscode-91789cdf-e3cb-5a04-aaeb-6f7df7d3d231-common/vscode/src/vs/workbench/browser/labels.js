...
import { isEqual, basenameOrAuthority, dirname } from '@codingame/monaco-vscode-api/vscode/vs/base/common/resources';
...
import { FILES_ASSOCIATIONS_CONFIG, FileKind } from '@codingame/monaco-vscode-api/vscode/vs/platform/files/common/files';
    ...
    setResource(label, options = Object.create(null)) {
        ...
        if (!options.forceLabel && !isSideBySideEditor && resource?.scheme === Schemas.vscodeNotebookCell) {
            ...
            if (notebookDocument && cellIndex !== undefined && typeof label.name === 'string') {
                options.title = ( localize(2742, "{0} • Cell {1}", label.name, `${cellIndex + 1}`));
            }
            if (typeof label.name === 'string' && notebookDocument && cellIndex !== undefined && typeof label.name === 'string') {
                label.name = ( localize(2742, "{0} • Cell {1}", label.name, `${cellIndex + 1}`));
            }
        }
