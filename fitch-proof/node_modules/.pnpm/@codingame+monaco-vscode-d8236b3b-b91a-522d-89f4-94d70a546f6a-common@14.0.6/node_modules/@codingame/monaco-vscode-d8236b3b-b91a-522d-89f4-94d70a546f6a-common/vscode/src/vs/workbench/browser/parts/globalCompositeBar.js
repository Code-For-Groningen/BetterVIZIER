...
import { addDisposableListener, EventType, EventHelper, getWindow, runWhenWindowIdle, append, $, clearNode, hide, show } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
...
import { AnchorAxisAlignment, AnchorAlignment } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/ui/contextview/contextview';
...
import { ACTIVITY_BAR_BADGE_FOREGROUND, ACTIVITY_BAR_BADGE_BACKGROUND } from '@codingame/monaco-vscode-api/vscode/vs/workbench/common/theme';
...
let GlobalCompositeBar = class GlobalCompositeBar extends Disposable {
    ...
    static { this.ACCOUNTS_ICON = registerIcon('accounts-view-bar-icon', Codicon.account, ( localize(3246, "Accounts icon in the view bar."))); }
    constructor(contextMenuActionsProvider, colors, activityHoverOptions, configurationService, instantiationService, storageService, extensionService) {
        ...
        this.globalActivityActionBar = this._register(( new ActionBar(this.element, {
            actionViewItemProvider: (action, options) => {
                ...
                if (action.id === ACCOUNTS_ACTIVITY_ID) {
                    return this.instantiationService.createInstance(AccountsActivityActionViewItem, this.contextMenuActionsProvider, {
                        ...
                        actions.unshift(...[
                            toAction({ id: 'hideAccounts', label: ( localize(3247, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }),
                            ( new Separator())
                        ]);
                    });
                }
            ...
            ariaLabel: ( localize(3248, "Manage")),
    ...
    getContextMenuActions() {
        return [toAction({ id: 'toggleAccountsVisibility', label: ( localize(3249, "Accounts")), checked: this.accountsVisibilityPreference, run: () => this.accountsVisibilityPreference = !this.accountsVisibilityPreference })];
    }
...
let AccountsActivityActionViewItem = class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    async resolveMainMenuActions(accountsMenu, disposables) {
        ...
        for (const providerId of providers) {
            if (!this.initialized) {
                const noAccountsAvailableAction = disposables.add(( new Action('noAccountsAvailable', ( localize(3250, "Loading...")), undefined, false)));
            ...
            if (!accounts) {
                if (( this.problematicProviders.has(providerId))) {
                    const providerUnavailableAction = disposables.add(( new Action('providerUnavailable', ( localize(3251, '{0} is currently unavailable', providerLabel)), undefined, false)));
            ...
            for (const account of accounts) {
                const manageExtensionsAction = toAction({
                    ...
                    label: ( localize(3252, "Manage Trusted Extensions")),
                    enabled: true,
        ...
        if (providers.length && !menus.length) {
            const noAccountsAvailableAction = disposables.add(( new Action('noAccountsAvailable', ( localize(3254, "You are not signed in to any accounts")), undefined, false)));
            menus.push(noAccountsAvailableAction);
        }
...
function simpleActivityContextMenuActions(storageService, isAccount) {
    ...
    if (isAccount) {
        currentElementContextMenuActions.push(toAction({ id: 'hideAccounts', label: ( localize(3247, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }), ( new Separator()));
    }
    return [
        ...currentElementContextMenuActions,
        toAction({ id: 'toggle.hideAccounts', label: ( localize(3249, "Accounts")), checked: isAccountsActionVisible(storageService), run: () => setAccountsActionVisible(storageService, !isAccountsActionVisible(storageService)) }),
        toAction({ id: 'toggle.hideManage', label: ( localize(3248, "Manage")), checked: true, enabled: false, run: () => { throw ( new Error('"Manage" can not be hidden')); } })
    ];
}
