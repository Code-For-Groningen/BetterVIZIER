...
let TerminalEditorInput = class TerminalEditorInput extends EditorInput {
    ...
    async confirm(terminals) {
        const { confirmed } = await this._dialogService.confirm({
            ...
            message: ( localize(9634, "Do you want to terminate running processes?")),
            primaryButton: ( localize(9635, "&&Terminate")),
            detail: terminals.length > 1 ?
                ( terminals.map(terminal => terminal.editor.getName())).join('\n') + '\n\n' + ( localize(9636, "Closing will terminate the running processes in the terminals.")) :
                ( localize(9637, "Closing will terminate the running processes in this terminal."))
        });
