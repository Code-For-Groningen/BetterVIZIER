    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                ...
                this.notificationService.warn(( localize(1290, 'Failed to open this link because its target is missing.')));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? ( localize(1291, 'Execute command'))
            : ( localize(1292, 'Follow link'));
    const kb = useMetaKey
        ? isMacintosh
            ? ( localize(1293, "cmd + click"))
            : ( localize(1294, "ctrl + click"))
        : isMacintosh
            ? ( localize(1295, "option + click"))
            : ( localize(1296, "alt + click"));
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(( link.url.toString()))) {
            ...
            if (match) {
                ...
                nativeLabel = ( localize(1297, "Execute command {0}", commandId));
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1298, "Open Link")),
