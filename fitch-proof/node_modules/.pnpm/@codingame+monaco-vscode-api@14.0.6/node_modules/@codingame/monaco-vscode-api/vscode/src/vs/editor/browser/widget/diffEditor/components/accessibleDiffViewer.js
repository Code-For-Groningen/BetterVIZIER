...
let View = class View extends Disposable {
    constructor(_element, _model, _width, _height, _models, _languageService) {
        ...
        this._register(autorun(reader => {
            ...
            if (this._model.canClose.read(reader)) {
                this._actionBar.push(( new Action('diffreview.close', ( localize(164, "Close")), 'close-diff-review ' + ThemeIcon.asClassName(accessibleDiffViewerCloseIcon), true, async () => _model.close())), { label: false, icon: true });
            }
        }));
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', ( localize(165, 'Accessible Diff Viewer. Use arrow up and down to navigate.')));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? ( localize(166, "no lines changed"))
                    : lines === 1 ? ( localize(167, "1 line changed"))
                        : ( localize(168, "{0} lines changed", lines));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = ( localize(170, "blank"));
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = ( localize(171, "{0} unchanged line {1}", lineContent, item.originalLineNumber));
                }
                else {
                    ariaLabel = ( localize(
                        172,
                ...
                break;
            case LineType.Added:
                ariaLabel = ( localize(173, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber));
                break;
            case LineType.Deleted:
                ariaLabel = ( localize(174, "- {0} original line {1}", lineContent, item.originalLineNumber));
                break;
        }
