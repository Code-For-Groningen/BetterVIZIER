    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return (new AsyncIterableObject(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(( new MarkdownHover(this, anchor.range, [( new MarkdownString()).appendText(( localize(1188, "Double-click to insert")))], false, 10001)));
            }
            let partTooltip;
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ? isMacintosh
                        ? ( localize(1189, "cmd + click"))
                        : ( localize(1190, "ctrl + click"))
                    : isMacintosh
                        ? ( localize(1191, "option + click"))
                        : ( localize(1192, "alt + click"));
                if (part.part.location && part.part.command) {
                    linkHint = ( new MarkdownString()).appendText(( localize(1193, 'Go to Definition ({0}), right click for more', kb)));
                }
                else if (part.part.location) {
                    linkHint = ( new MarkdownString()).appendText(( localize(1194, 'Go to Definition ({0})', kb)));
                }
                else if (part.part.command) {
                    linkHint = ( new MarkdownString(`[${( localize(1195, "Execute Command"))}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true }));
                }
                if (linkHint) {
