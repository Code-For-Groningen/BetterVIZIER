...
let InlineCompletionsSource = class InlineCompletionsSource extends Disposable {
    ...
    fetch(position, context, activeInlineCompletion) {
        ...
        const promise = (async () => {
            ...
            if (activeInlineCompletion && activeInlineCompletion.isInlineEdit && ((activeInlineCompletion.canBeReused(this._textModel, position) || ( updatedCompletions.has(activeInlineCompletion.inlineCompletion))) )) {
                updatedCompletions.dispose();
                return false;
            }
...
class UpToDateInlineCompletions {
    ...
    constructor(inlineCompletionProviderResult, request, _textModel, _versionId, _invalidationDelay) {
        ...
        this._inlineCompletions = ( inlineCompletionProviderResult.completions.map((i, index) => ( new InlineCompletionWithUpdatedRange(
            i,
            ids[index],
            this._textModel,
            this._versionId,
    ...
    prepend(inlineCompletion, range, addRefToSource) {
        ...
        this._inlineCompletions.unshift(( new InlineCompletionWithUpdatedRange(
            inlineCompletion,
            id,
            this._textModel,
            this._versionId,
...
class InlineCompletionWithUpdatedRange {
    ...
    _toIndividualEdits(editRange, _replaceText) {
        ...
        if (innerChanges.length === 0) {
            ...
            return ( new OffsetEdit([( new SingleOffsetEdit(
                OffsetRange.ofStartAndLength(startOffset, editOriginalText.length),
                editReplaceText
            ))]));
        }
    ...
    acceptTextModelChangeEvent(e, tx) {
        ...
        const changePartiallyAcceptsEdit = ( editUpdates.some(
            ({ changeType }) => changeType === 'partiallyAccepted' || changeType === 'fullyAccepted'
        ));
...
export { InlineCompletionWithUpdatedRange, InlineCompletionsSource, StructuredLogger, UpToDateInlineCompletions, formatRecordableLogEntry, observableContextKey };
