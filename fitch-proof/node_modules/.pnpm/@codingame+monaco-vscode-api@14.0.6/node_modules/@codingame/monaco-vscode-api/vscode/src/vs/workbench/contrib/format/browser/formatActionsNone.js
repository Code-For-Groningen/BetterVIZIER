...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(6896, "Format Document")),
    ...
    async run(accessor, editor) {
        ...
        if (formatterCount > 1) {
            ...
            notificationService.warn(( localize(6897, "This file cannot be formatted because it is too large")));
        }
        else {
            const langName = model.getLanguageId();
            const message = ( localize(6898, "There is no formatter for '{0}' files installed.", langName));
            const { confirmed } = await dialogService.confirm({
                message,
                primaryButton: ( localize(6899, "&&Install Formatter..."))
            });
            if (confirmed) {
