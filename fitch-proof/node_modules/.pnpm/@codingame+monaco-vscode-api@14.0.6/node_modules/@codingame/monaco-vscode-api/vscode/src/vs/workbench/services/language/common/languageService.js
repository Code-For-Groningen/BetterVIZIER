...
const languagesExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(11862, 'Contributes language declarations.')),
        type: 'array',
        items: {
            type: 'object',
            ...
            properties: {
                id: {
                    description: ( localize(11863, 'ID of the language.')),
                    type: 'string'
                },
                aliases: {
                    description: ( localize(11864, 'Name aliases for the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                extensions: {
                    description: ( localize(11865, 'File extensions associated to the language.')),
                    default: ['.foo'],
                    type: 'array',
                    items: {
                        type: 'string'
                ...
                filenames: {
                    description: ( localize(11866, 'File names associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                filenamePatterns: {
                    description: ( localize(11867, 'File name glob patterns associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                mimetypes: {
                    description: ( localize(11868, 'Mime types associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                icon: {
                    ...
                    properties: {
                        light: {
                            description: ( localize(11872, 'Icon path when a light theme is used')),
                            type: 'string'
                        },
                        dark: {
                            description: ( localize(11873, 'Icon path when a dark theme is used')),
                            type: 'string'
                        }
                    }
                }
...
class LanguageTableRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(11874, "ID")),
            ( localize(11875, "Name")),
            ( localize(11876, "File Extensions")),
            ( localize(11877, "Grammar")),
            ( localize(11878, "Snippets"))
        ];
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: 'languages',
    label: ( localize(11879, "Programming Languages")),
    access: {
        canToggle: false
    },
...
function isValidLanguageExtensionPoint(value, collector) {
    if (!value) {
        collector?.error(( localize(11881, "Empty value for `contributes.{0}`", languagesExtPoint.name)));
        return false;
    }
    if (typeof value.id !== 'string') {
        collector?.error(( localize(11882, "property `{0}` is mandatory and must be of type `string`", 'id')));
        return false;
    }
