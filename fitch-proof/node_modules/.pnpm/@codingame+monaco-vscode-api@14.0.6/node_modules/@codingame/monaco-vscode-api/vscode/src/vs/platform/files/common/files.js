...
function ensureFileSystemProviderError(error) {
    if (!error) {
        return createFileSystemProviderError(( localize(1808, "Unknown Error")), FileSystemProviderErrorCode.Unknown);
    }
    return error;
}
...
class ByteSize {
    ...
    static formatSize(size) {
        ...
        if (size < ByteSize.KB) {
            return localize(1809, "{0}B", size.toFixed(0));
        }
        if (size < ByteSize.MB) {
            return localize(1810, "{0}KB", (size / ByteSize.KB).toFixed(2));
        }
        if (size < ByteSize.GB) {
            return localize(1811, "{0}MB", (size / ByteSize.MB).toFixed(2));
        }
        if (size < ByteSize.TB) {
            return localize(1812, "{0}GB", (size / ByteSize.GB).toFixed(2));
        }
        return localize(1813, "{0}TB", (size / ByteSize.TB).toFixed(2));
    }
}
