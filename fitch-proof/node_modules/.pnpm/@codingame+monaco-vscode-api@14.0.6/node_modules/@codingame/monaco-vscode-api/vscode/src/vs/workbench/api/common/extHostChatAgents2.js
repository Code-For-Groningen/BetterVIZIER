        ...
        if (!this._apiObject) {
            ...
            this._apiObject = ( Object.freeze({
                ...
                textEdit(target, edits) {
                    ...
                    checkProposedApiEnabled(that._extension, 'chatParticipantAdditions');
                    const part = new ChatResponseTextEditPart(target, Array.isArray(edits) ? edits : []);
                ...
                detectedParticipant(participant, command) {
                    ...
                    checkProposedApiEnabled(that._extension, 'chatParticipantAdditions');
                    const part = new ChatResponseDetectedParticipantPart(participant, command);
                    const dto = ChatResponseDetectedParticipantPart$1.from(part);
                    _report(dto);
                    return this;
                },
                ...
                push(part) {
                    ...
                    if (part instanceof ChatResponseTextEditPart ||
                        ...
                        part instanceof ChatResponseDetectedParticipantPart ||
...
class ExtHostChatAgents2 extends Disposable {
    ...
    async $detectChatParticipant(handle, requestDto, context, options, token) {
        ...
        const extRequest = ChatAgentRequest.to(request, location, model, isProposedApiEnabled(detector.extension, 'chatReadonlyPromptReference'));
    ...
    async $invokeAgent(handle, requestDto, context, token) {
        ...
        try {
            ...
            const extRequest = ChatAgentRequest.to(request, location, model, isProposedApiEnabled(agent.extension, 'chatReadonlyPromptReference'));
    ...
    async prepareHistoryTurns(extension, agentId, context) {
        ...
        for (const h of context.history) {
            ...
            const varsWithoutTools = ( h.request.variables.variables
                .filter(v => !v.isTool)
                .map(v => ChatPromptReference.to(v, hasReadonlyProposal)));
