...
import { unthemedInboxStyles, MessageType } from '../inputbox/inputBox.js';
import { ElementsDragAndDropData } from '../list/listView.js';
import { isStickyScrollContainer, isStickyScrollElement, MouseController, isButton, isMonacoEditor, isMonacoCustomToggle, isActionItem, List } from '../list/listWidget.js';
import { Toggle, unthemedToggleStyles } from '../toggle/toggle.js';
import { isFilterResult, getVisibleState } from './indexTreeModel.js';
import { TreeDragOverBubble, TreeVisibility, TreeMouseEventTarget, TreeError } from './tree.js';
import { Action } from '../../../common/actions.js';
import { range, equals, distinct, insertInto } from '../../../common/arrays.js';
import { disposableTimeout, timeout, Delayer } from '../../../common/async.js';
import { Codicon } from '../../../common/codicons.js';
import { ThemeIcon } from '../../../common/themables.js';
import { SetMap } from '../../../common/map.js';
import { Event, Emitter, EventBufferer, Relay } from '../../../common/event.js';
...
class FindWidget extends Disposable {
    ...
    constructor(container, tree, contextViewProvider, placeholder, toggleContributions = [], options) {
        ...
        this.findInput = this._register(( new FindInput(this.elements.findInput, contextViewProvider, {
            label: ( localize(32, "Type to search")),
            placeholder,
        ...
        const closeAction = this._register(( new Action('close', ( localize(33, "Close")), 'codicon codicon-close', true, () => this.dispose())));
...
class AbstractFindController {
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        this._placeholder = options.placeholder ?? ( localize(32, "Type to search"));
    }
    isOpened() {
        return !!this.widget;
    }
    ...
    renderMessage(showNotFound, warningMessage) {
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({ type: MessageType.WARNING, content: warningMessage ?? ( localize(34, "No results found.")) });
            }
            else {
    ...
    alertResults(results) {
        if (!results) {
            alert(( localize(35, "No results")));
        }
        else {
            alert(( localize(36, "{0} results", results)));
        }
    }
    dispose() {
...
class FindController extends AbstractFindController {
    ...
    set mode(mode) {
        ...
        this.placeholder = isFilterMode ? ( localize(37, "Type to filter")) : ( localize(32, "Type to search"));
...
class AbstractTree {
    ...
    constructor(_user, container, delegate, renderers, _options = {}) {
        ...
        this.view = ( new TreeNodeList(
            ...
            this.selection,
            this.anchor,
            { ...asListOptions(() => this.model, this.disposables, _options), tree: this, stickyScrollProvider: () => this.stickyScrollController }
        ));
...
export { AbstractFindController, AbstractTree, AbstractTreePart, AbstractTreeViewState, ComposedTreeDelegate, FindController, FindFilter, FindToggles, RenderIndentGuides, TreeFindMatchType, TreeFindMode, TreeRenderer, contiguousFuzzyScore };
