    ...
    async _handleMissingInstalledDependency(extension, missingInstalledDependency) {
        ...
        if (this._extensionEnablementService.isEnabled(missingInstalledDependency)) {
            this._notificationService.notify({
                ...
                actions: {
                    primary: [( new Action('reload', ( localize(2318, "Reload Window")), '', true, () => this._hostService.reload()))]
                }
            });
        }
        else {
            ...
            if (enablementState === EnablementState.DisabledByVirtualWorkspace) {
                ...
                this._notificationService.notify({
                    ...
                    actions: {
                        primary: [( new Action('manageWorkspaceTrust', ( localize(2321, "Manage Workspace Trust")), '', true, () => this._commandService.executeCommand('workbench.trust.manage')))]
                    }
                });
            }
                ...
                this._notificationService.notify({
                    ...
                    actions: {
                        primary: [( new Action('enable', ( localize(2323, "Enable and Reload")), '', true, () => this._extensionEnablementService.setEnablement([missingInstalledDependency], enablementState === EnablementState.DisabledGlobally ? EnablementState.EnabledGlobally : EnablementState.EnabledWorkspace)
    ...
    async _handleMissingNotInstalledDependency(extension, missingDependency) {
        ...
        if (dependencyExtension) {
            this._notificationService.notify({
                ...
                actions: {
                    primary: [( new Action('install', ( localize(2326, "Install and Reload")), '', true, () => this._extensionsWorkbenchService.install(dependencyExtension)
                            .then(() => this._hostService.reload(), e => this._notificationService.error(e))))]
                }
            });
        }
