...
let MainThreadOutputService = class MainThreadOutputService extends Disposable {
    ...
    _showChannelQuietly(channel) {
        const statusProperties = {
            name: ( localize(2352, "Show Output")),
            text: '$(output)',
            ariaLabel: ( localize(2353, "Show {0} Output Channel", channel.label)),
            command: `workbench.action.output.show.${channel.id}`,
            tooltip: ( localize(2354, "Show {0} Output Channel", channel.label)),
            kind: 'prominent'
        };
        if (!this._outputStatusItem.value) {
            this._outputStatusItem.value = this._statusbarService.addEntry(statusProperties, 'status.view.showQuietly', StatusbarAlignment.RIGHT, { id: 'status.notifications', alignment: StatusbarAlignment.LEFT });
        }
        else {
