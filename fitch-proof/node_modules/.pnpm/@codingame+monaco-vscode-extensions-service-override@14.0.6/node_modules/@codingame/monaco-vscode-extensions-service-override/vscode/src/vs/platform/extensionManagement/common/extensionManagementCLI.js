...
const notFound = (id) => ( localize(1717, "Extension '{0}' not found.", id));
...
let ExtensionManagementCLI = class ExtensionManagementCLI {
    ...
    async listExtensions(showVersions, category, profileLocation) {
        ...
        if (this.location) {
            this.logger.info(( localize(1719, "Extensions installed on {0}:", this.location)));
        }
    ...
    async installExtensions(extensions, builtinExtensions, installOptions, force) {
        ...
        try {
            if (extensions.length) {
                this.logger.info(this.location ? ( localize(1720, "Installing extensions on {0}...", this.location)) : ( localize(1721, "Installing extensions...")));
            }
            ...
            this.logger.error(( localize(1722, "Error while installing extensions: {0}", getErrorMessage(error))));
            throw error;
        }
        if (failed.length) {
            throw ( new Error(( localize(1723, "Failed Installing Extensions: {0}", failed.join(', ')))));
        }
    }
    async updateExtensions(profileLocation) {
        ...
        if (!extensionsToUpdate.length) {
            this.logger.info(( localize(1725, "No extension to update")));
            return;
        }
    ...
    async installGalleryExtensions(installExtensionInfos, installed, force) {
        installExtensionInfos = installExtensionInfos.filter(({ id, version }) => {
            ...
            if (installedExtension) {
                ...
                if (version && installedExtension.manifest.version === version) {
                    this.logger.info(( localize(1730, "Extension '{0}' is already installed.", `${id}@${version}`)));
                    return false;
                }
            }
            return true;
        ...
        await Promise.all(( installExtensionInfos.map(async ({ id, version, installOptions }) => {
            ...
            if (installOptions.isBuiltin) {
                this.logger.info(version ? ( localize(1732, "Installing builtin extension '{0}' v{1}...", id, version)) : ( localize(1733, "Installing builtin extension '{0}'...", id)));
            }
            else {
                this.logger.info(version ? ( localize(1734, "Installing extension '{0}' v{1}...", id, version)) : ( localize(1735, "Installing extension '{0}'...", id)));
            }
    ...
    async installVSIX(vsix, installOptions, force, installedExtensions) {
        ...
        if (valid) {
            try {
                ...
                this.logger.info(( localize(1738, "Extension '{0}' was successfully installed.", basename(vsix))));
            }
            catch (error) {
                if (isCancellationError(error)) {
                    this.logger.info(( localize(1739, "Cancelled installing extension '{0}'.", basename(vsix))));
                }
                else {
                    throw error;
                }
    ...
    async uninstallExtensions(extensions, force, profileLocation) {
        ...
        for (const extension of extensions) {
            ...
            this.logger.info(( localize(1743, "Uninstalling {0}...", id)));
            ...
            if (this.location) {
                ...
                this.logger.info(( localize(1745, "Extension '{0}' was successfully uninstalled!", id)));
            }
        }
    }
    ...
    notInstalled(id) {
        return this.location ? ( localize(1746, "Extension '{0}' is not installed on {1}.", id, this.location)) : ( localize(1747, "Extension '{0}' is not installed.", id));
    }
};
