...
let ExtensionBisectUi = class ExtensionBisectUi {
    ...
    _showBisectPrompt() {
        const goodPrompt = {
            label: ( localize(11607, "I can't reproduce")),
            run: () => this._commandService.executeCommand('extension.bisect.next', false)
        };
        const badPrompt = {
            label: ( localize(11608, "I can reproduce")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11611, 'Start Extension Bisect')),
    ...
    async run(accessor) {
        ...
        const res = await dialogService.confirm({
            message: ( localize(11612, "Extension Bisect")),
            detail: ( localize(
                11613,
            ...
            primaryButton: ( localize(11614, "&&Start Extension Bisect"))
        });
        if (res.confirmed) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11615, 'Continue Extension Bisect')),
            category: Categories.Help,
            f1: true,
    ...
    async run(accessor, seeingBad) {
        ...
        if (done.bad) {
            await dialogService.info(( localize(11616, "Extension Bisect")), ( localize(
                11617,
            ...
            const res = await dialogService.confirm({
                type: Severity.Info,
                message: ( localize(11616, "Extension Bisect")),
                primaryButton: ( localize(11618, "&&Report Issue & Continue")),
                cancelButton: ( localize(11619, "Continue")),
                detail: ( localize(
                    11620,
                    "Extension Bisect is done and has identified {0} as the extension causing the problem.",
                    done.id
                )),
                checkbox: { label: ( localize(11621, "Keep this extension disabled")), checked: true }
            });
    ...
    async _checkForBad(dialogService, bisectService) {
        const { result } = await dialogService.prompt({
            type: Severity.Info,
            message: ( localize(11622, "Extension Bisect")),
            detail: ( localize(
                11623,
                "Extension Bisect is active and has disabled {0} extensions. Check if you can still reproduce the problem and proceed by selecting from these options.",
            ...
            buttons: [
                {
                    label: ( localize(11624, "I ca&&n't reproduce")),
                    run: () => false
                },
                {
                    label: ( localize(11625, "I can &&reproduce")),
                    run: () => true
                },
                {
                    label: ( localize(11626, "&&Stop Bisect")),
                    run: () => undefined
                }
            ],
            cancelButton: {
                label: ( localize(11627, "&&Cancel Bisect")),
                run: () => null
            }
        });
        return result;
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11628, 'Stop Extension Bisect')),
            category: Categories.Help,
            f1: true,
            precondition: ExtensionBisectUi.ctxIsBisectActive
        });
