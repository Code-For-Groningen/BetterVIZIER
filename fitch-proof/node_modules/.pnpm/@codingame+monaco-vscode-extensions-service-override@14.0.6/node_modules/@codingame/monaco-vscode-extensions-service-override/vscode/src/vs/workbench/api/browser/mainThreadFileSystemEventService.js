                ...
                if (showPreview === undefined) {
                    ...
                    if (needsConfirmation) {
                        const { confirmed } = await dialogService.confirm({
                            type: Severity.Info,
                            message,
                            primaryButton: ( localize(2336, "Show &&Preview")),
                            cancelButton: ( localize(2337, "Skip Changes"))
                        });
                        showPreview = true;
                        if (!confirmed) {
                            return;
                        ...
                        const { result, checkboxChecked } = await dialogService.prompt({
                            ...
                            buttons: [
                                {
                                    label: ( localize(2338, "&&OK")),
                                    run: () => Choice.OK
                                },
                                {
                                    label: ( localize(2336, "Show &&Preview")),
                                    run: () => Choice.Preview
                                }
                            ],
                            cancelButton: {
                                label: ( localize(2337, "Skip Changes")),
                                run: () => Choice.Cancel
                            },
                            checkbox: { label: ( localize(2339, "Do not ask me again")) }
                        });
            ...
            _progressLabel(operation) {
                switch (operation) {
                    case FileOperation.CREATE:
                        return localize(2340, "Running 'File Create' participants...");
                    case FileOperation.MOVE:
                        return localize(2341, "Running 'File Rename' participants...");
                    case FileOperation.COPY:
                        return localize(2342, "Running 'File Copy' participants...");
                    case FileOperation.DELETE:
                        return localize(2343, "Running 'File Delete' participants...");
                    case FileOperation.WRITE:
                        return localize(2344, "Running 'File Write' participants...");
                }
            }
        });
...
registerAction2(class ResetMemento extends Action2 {
    constructor() {
        super({
            ...
            title: {
                value: ( localize(2345, "Reset choice for 'File operation needs preview'")),
