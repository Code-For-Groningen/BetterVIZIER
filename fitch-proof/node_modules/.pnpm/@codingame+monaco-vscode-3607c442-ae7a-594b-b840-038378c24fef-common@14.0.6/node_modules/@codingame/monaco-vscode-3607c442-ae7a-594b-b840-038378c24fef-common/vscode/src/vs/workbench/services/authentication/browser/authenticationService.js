...
const authenticationDefinitionSchema = {
    ...
    properties: {
        id: {
            type: 'string',
            description: ( localize(11466, 'The id of the authentication provider.'))
        },
        label: {
            type: 'string',
            description: ( localize(11467, 'The human readable name of the authentication provider.')),
        }
    }
};
const authenticationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(11468, 'Contributes authentication')),
        type: 'array',
...
let AuthenticationService = class AuthenticationService extends Disposable {
            ...
            added.forEach(point => {
                for (const provider of point.value) {
                    if (isFalsyOrWhitespace(provider.id)) {
                        point.collector.error(( localize(11469, 'An authentication contribution must specify an id.')));
                        continue;
                    }
                    if (isFalsyOrWhitespace(provider.label)) {
                        point.collector.error(( localize(11470, 'An authentication contribution must specify a label.')));
                        continue;
                    }
    ...
    registerDeclaredAuthenticationProvider(provider) {
        if (isFalsyOrWhitespace(provider.id)) {
            throw ( new Error(( localize(11469, 'An authentication contribution must specify an id.'))));
        }
        if (isFalsyOrWhitespace(provider.label)) {
            throw ( new Error(( localize(11470, 'An authentication contribution must specify a label.'))));
        }
        if (( this.declaredProviders.some(p => p.id === provider.id))) {
