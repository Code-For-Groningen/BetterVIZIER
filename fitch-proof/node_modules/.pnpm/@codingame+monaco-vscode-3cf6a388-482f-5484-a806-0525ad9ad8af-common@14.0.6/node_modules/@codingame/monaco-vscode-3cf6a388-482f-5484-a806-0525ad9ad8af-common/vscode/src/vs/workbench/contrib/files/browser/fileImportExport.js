...
import { ByteSize, FileSystemProviderCapabilities } from '@codingame/monaco-vscode-api/vscode/vs/platform/files/common/files';
...
import { VIEW_ID, UndoConfirmLevel } from '@codingame/monaco-vscode-eea37658-2307-56a5-8ed0-b2752c7b7a50-common/vscode/vs/workbench/contrib/files/common/files';
import { IEditorService } from '@codingame/monaco-vscode-api/vscode/vs/workbench/services/editor/common/editorService.service';
import { RunOnceWorker, Limiter, Promises } from '@codingame/monaco-vscode-api/vscode/vs/base/common/async';
import { newWriteableBufferStream, VSBuffer } from '@codingame/monaco-vscode-api/vscode/vs/base/common/buffer';
import { joinPath, basename, dirname } from '@codingame/monaco-vscode-api/vscode/vs/base/common/resources';
import { ResourceFileEdit } from '@codingame/monaco-vscode-api/vscode/vs/editor/browser/services/bulkEditService';
import { ExplorerItem } from '@codingame/monaco-vscode-eea37658-2307-56a5-8ed0-b2752c7b7a50-common/vscode/vs/workbench/contrib/files/common/explorerModel';
import { URI } from '@codingame/monaco-vscode-api/vscode/vs/base/common/uri';
import { IHostService } from '@codingame/monaco-vscode-api/vscode/vs/workbench/services/host/browser/host.service';
import { IWorkspaceContextService } from '@codingame/monaco-vscode-api/vscode/vs/platform/workspace/common/workspace.service';
import { extractEditorsAndFilesDropData } from '@codingame/monaco-vscode-api/vscode/vs/platform/dnd/browser/dnd';
...
import { isDragEvent, getActiveWindow, triggerDownload } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(
                        ...
                        undoLabel: ( localize(6680, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(6681, "Overwriting {0}", entry.name)),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [
                {
                    label: folders.length > 1 ?
                        ( localize(6685, "&&Copy Folders")) :
                        ( localize(6686, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ?
                        ( localize(6687, "&&Add Folders to Workspace")) :
                        ( localize(6688, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ?
                    ( localize(6691, "Are you sure to want to copy folders?")) :
                    ( localize(6692, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? ( localize(6693, "Some or all of the dropped files could not be accessed for import.")) : ( localize(6694, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    ( localize(6695, "Import {0}", basename(resourcesFiltered[0]))) :
                    ( localize(6696, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ?
                    ( localize(6697, "Copying {0}", basename(resourcesFiltered[0]))) :
                    ( localize(6698, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(6699, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(6702, "Download")),
            title: ( localize(6703, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true }))], {
                undoLabel: ( localize(6704, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(6705, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        }
        else {
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(6707, "This action is irreversible!")),
        primaryButton: ( localize(6708, "&&Replace")),
        type: 'warning'
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + '\n' + ( localize(6707, "This action is irreversible!")),
            primaryButton: ( localize(6708, "&&Replace")),
            type: 'warning'
        };
    }
