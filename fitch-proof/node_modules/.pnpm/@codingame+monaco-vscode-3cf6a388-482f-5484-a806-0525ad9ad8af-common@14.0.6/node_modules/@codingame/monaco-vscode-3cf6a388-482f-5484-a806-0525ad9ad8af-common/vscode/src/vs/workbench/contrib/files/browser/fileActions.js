...
import { isWindows, OS } from '@codingame/monaco-vscode-api/vscode/vs/base/common/platform';
import { extname, basename, isAbsolute } from '@codingame/monaco-vscode-api/vscode/vs/base/common/path';
import { distinctParents, basenameOrAuthority, joinPath, basename as basename$1, isEqualOrParent } from '@codingame/monaco-vscode-api/vscode/vs/base/common/resources';
...
import { VIEWLET_ID, VIEW_ID, UndoConfirmLevel } from '@codingame/monaco-vscode-eea37658-2307-56a5-8ed0-b2752c7b7a50-common/vscode/vs/workbench/contrib/files/common/files';
...
import { SAVE_ALL_IN_GROUP_COMMAND_ID, REVEAL_IN_EXPLORER_COMMAND_ID, NEW_UNTITLED_FILE_COMMAND_ID } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/files/browser/fileConstants';
...
import { getFileNamesMessage } from '@codingame/monaco-vscode-d4a98668-f793-5361-a9cf-1e6ae6d62ce3-common/vscode/vs/platform/dialogs/common/dialogs';
...
import { CLOSE_EDITORS_AND_GROUP_COMMAND_ID } from '@codingame/monaco-vscode-f48982c4-9e82-55e2-b800-20e6d1e6096f-common/vscode/vs/workbench/browser/parts/editor/editorCommands';
import { coalesce } from '@codingame/monaco-vscode-api/vscode/vs/base/common/arrays';
import { NewExplorerItem } from '@codingame/monaco-vscode-eea37658-2307-56a5-8ed0-b2752c7b7a50-common/vscode/vs/workbench/contrib/files/common/explorerModel';
import { getErrorMessage } from '@codingame/monaco-vscode-api/vscode/vs/base/common/errors';
import { triggerUpload } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
...
import { trim, rtrim } from '@codingame/monaco-vscode-api/vscode/vs/base/common/strings';
...
import { ActiveEditorContext, EmptyWorkspaceSupportContext, ActiveEditorCanToggleReadonlyContext } from '@codingame/monaco-vscode-api/vscode/vs/workbench/common/contextkeys';
...
import { getPathForFile } from '@codingame/monaco-vscode-api/vscode/vs/platform/dnd/browser/dnd';
import Severity from '@codingame/monaco-vscode-api/vscode/vs/base/common/severity';

const NEW_FILE_COMMAND_ID = 'explorer.newFile';
const NEW_FILE_LABEL = ( localize2(6572, "New File..."));
const NEW_FOLDER_COMMAND_ID = 'explorer.newFolder';
const NEW_FOLDER_LABEL = ( localize2(6573, "New Folder..."));
const TRIGGER_RENAME_LABEL = ( localize(6574, "Rename..."));
const MOVE_FILE_TO_TRASH_LABEL = ( localize(6575, "Delete"));
const COPY_FILE_LABEL = ( localize(6576, "Copy"));
const PASTE_FILE_LABEL = ( localize(6577, "Paste"));
const FileCopiedContext = ( new RawContextKey('fileCopied', false));
const DOWNLOAD_COMMAND_ID = 'explorer.download';
const DOWNLOAD_LABEL = ( localize(6578, "Download..."));
const UPLOAD_COMMAND_ID = 'explorer.upload';
const UPLOAD_LABEL = ( localize(6579, "Upload..."));
...
async function deleteFiles(explorerService, workingCopyFileService, dialogService, configurationService, filesConfigurationService, elements, useTrash, skipConfirm = false, ignoreIfNotExists = false) {
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? ( localize(6580, "&&Move to Recycle Bin")) : ( localize(6581, "&&Move to Trash"));
    }
    else {
        primaryButton = ( localize(6582, "&&Delete"));
    }
    ...
    if (dirtyWorkingCopies.size) {
        ...
        const response = await dialogService.confirm({
            type: 'warning',
            message,
            detail: ( localize(6587, "Your changes will be lost if you don't save them.")),
            primaryButton
        });
    ...
    if (!skipConfirm) {
        ...
        if (readonlyResources.length) {
            ...
            const response = await dialogService.confirm({
                type: 'warning',
                message,
                detail: ( localize(6591, "The read-only protection will be overridden if you continue.")),
                primaryButton: ( localize(6592, "Continue"))
            });
            if (!response.confirmed) {
                return;
            }
    ...
    let confirmation;
    const deleteDetail = ( distinctElements.some(e => e.isDirectory)) ? ( localize(6593, "This action is irreversible!")) :
        distinctElements.length > 1 ? ( localize(6594, "You can restore these files using the Undo command.")) : ( localize(6595, "You can restore this file using the Undo command."));
    if (skipConfirm || (useTrash && configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false)) {
        ...
        detail += detail ? '\n' : '';
        if (isWindows) {
            detail += distinctElements.length > 1 ? ( localize(6596, "You can restore these files from the Recycle Bin.")) : ( localize(6597, "You can restore this file from the Recycle Bin."));
        }
        else {
            detail += distinctElements.length > 1 ? ( localize(6598, "You can restore these files from the Trash.")) : ( localize(6599, "You can restore this file from the Trash."));
        }
        confirmation = await dialogService.confirm({
            ...
            primaryButton,
            checkbox: {
                label: ( localize(6600, "Do not ask me again"))
            }
        });
    }
    else {
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? ( localize(6601, "Delete {0} files", distinctElements.length)) : ( localize(6602, "Delete {0}", distinctElements[0].name)),
            progressLabel: distinctElements.length > 1 ? ( localize(6603, "Deleting {0} files", distinctElements.length)) : ( localize(6604, "Deleting {0}", distinctElements[0].name)),
        };
        ...
        if (useTrash) {
            ...
            primaryButton = ( localize(6607, "&&Delete Permanently"));
        }
        else {
            errorMessage = toErrorMessage(error, false);
            primaryButton = ( localize(6608, "&&Retry"));
        }
...
function getMoveToTrashMessage(distinctElements) {
    ...
    return { message: ( localize(6613, "Are you sure you want to delete '{0}'?", distinctElements[0].name)), detail: '' };
}
...
class GlobalCompareResourcesAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(6622, "Opens a picker to select a file to diff with the active editor."))
            }
        });
    }
    async run(accessor) {
...
class ToggleAutoSaveAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(6623, "Toggle Auto Save")),
            f1: true,
            category: Categories.File,
            metadata: { description: ( localize2(6624, "Toggle the ability to save files automatically after typing")) }
        });
    }
    run(accessor) {
...
class SaveAllInGroupAction extends BaseSaveAllAction {
    ...
    static { this.LABEL = ( localize(6625, "Save All in Group")); }
    get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
    ...
    static { this.LABEL = ( localize(6626, "Close Group")); }
...
class FocusFilesExplorer extends Action2 {
    ...
    static { this.LABEL = ( localize2(6627, "Focus on Files Explorer")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(6628, "Moves focus to the file explorer view container."))
            }
        });
    }
    async run(accessor) {
...
class ShowActiveFileInExplorer extends Action2 {
    ...
    static { this.LABEL = ( localize2(6629, "Reveal Active File in Explorer View")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(6630, "Reveals and selects the active file within the explorer view."))
            }
        });
    }
    async run(accessor) {
...
class OpenActiveFileInEmptyWorkspace extends Action2 {
    ...
    static { this.LABEL = ( localize2(6631, "Open Active File in New Empty Workspace")); }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(6632, "Opens the active file in a new window with no folders open."))
            }
        });
    }
    async run(accessor) {
        ...
        if (fileResource) {
            if (fileService.hasProvider(fileResource)) {
                ...
                dialogService.error(( localize(6633, "The active editor must contain an openable resource.")));
            }
        }
    }
}
function validateFileName(pathService, item, name, os) {
    ...
    if (!name || name.length === 0 || /^\s+$/.test(name)) {
        return {
            content: ( localize(6634, "A file or folder name must be provided.")),
            severity: Severity.Error
        };
    }
    if (name[0] === '/' || name[0] === '\\') {
        return {
            content: ( localize(6635, "A file or folder name cannot start with a slash.")),
            severity: Severity.Error
        };
    }
    ...
    if (( names.some(name => /^\s|\s$/.test(name)))) {
        return {
            content: ( localize(6638, "Leading or trailing whitespace detected in file or folder name.")),
...
class CompareNewUntitledTextFilesAction extends Action2 {
    ...
    static { this.LABEL = ( localize2(6639, "Compare New Untitled Text Files")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(6640, "Opens a new diff editor with two untitled files."))
            }
        });
    }
    async run(accessor) {
...
class CompareWithClipboardAction extends Action2 {
    ...
    static { this.LABEL = ( localize2(6641, "Compare Active File with Clipboard")); }
    ...
    async run(accessor) {
        ...
        if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
            ...
            const editorLabel = ( localize(6643, "Clipboard ↔ {0}", name));
            await editorService.openEditor({
    ...
    const onSuccess = async (value) => {
        try {
            ...
            await explorerService.applyBulkEdit([( new ResourceFileEdit(undefined, resourceToCreate, { folder: isFolder }))], {
                undoLabel: ( localize(6645, "Create {0}", value)),
                progressLabel: ( localize(6646, "Creating {0}", value)),
                ...
                if (( stat.resource.toString()) !== ( targetResource.toString())) {
                    try {
                        await explorerService.applyBulkEdit([( new ResourceFileEdit(stat.resource, targetResource))], {
                            ...
                            undoLabel: ( localize(6647, "Rename {0} to {1}", stat.name, value)),
                            progressLabel: ( localize(6648, "Renaming {0} to {1}", stat.name, value)),
                        });
                        await refreshIfSeparator(value, explorerService);
                    }
                    catch (e) {
...
const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        ...
        const confirmation = await dialogService.confirm({
            ...
            primaryButton: ( localize(6651, "&&Paste"))
        });
        if (!confirmation.confirmed) {
            return;
        }
        ...
        if (toPaste.type === 'paths') {
            const sourceTargetPairs = coalesce(await Promise.all(( toPaste.files.map(async (fileToPaste) => {
                if (( element.resource.toString()) !== ( fileToPaste.toString()) && isEqualOrParent(element.resource, fileToPaste)) {
                    throw ( new Error(( localize(6652, "File to paste is an ancestor of the destination folder"))));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === UndoConfirmLevel.Verbose,
                        progressLabel: sourceTargetPairs.length > 1 ? ( localize(6653, "Moving {0} files", sourceTargetPairs.length))
                            : ( localize(6654, "Moving {0}", basenameOrAuthority(sourceTargetPairs[0].target))),
                        undoLabel: sourceTargetPairs.length > 1 ? ( localize(6655, "Move {0} files", sourceTargetPairs.length))
                            : ( localize(6656, "Move {0}", basenameOrAuthority(sourceTargetPairs[0].target)))
                    };
                    await explorerService.applyBulkEdit(resourceFileEdits, options);
                }
                else {
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? ( localize(6658, "Copying {0} files", targets.length))
                : ( localize(6659, "Copying {0}", basenameOrAuthority(targets[0]))),
            undoLabel: targets.length > 1 ? ( localize(6660, "Paste {0} files", targets.length))
                : ( localize(6661, "Paste {0}", basenameOrAuthority(targets[0])))
        };
        await explorerService.applyBulkEdit(resourceFileEdits, options);
    }
};
...
class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(6662, "Set Active Editor Read-only in Session")); }
    constructor() {
...
class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(6663, "Set Active Editor Writeable in Session")); }
    constructor() {
...
class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(6664, "Toggle Active Editor Read-only in Session")); }
    constructor() {
...
class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(6665, "Reset Active Editor Read-only in Session")); }
    constructor() {
