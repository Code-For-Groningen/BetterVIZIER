        ...
        if (someDirty) {
            ...
            const buttons = [
                {
                    label: hasUnhandledConflicts ?
                        ( localize(7345, "&&Save With Conflicts")) :
                        ( localize(7346, "&&Save")),
                    run: () => ConfirmResult.SAVE
                },
                {
                    label: ( localize(7347, "Do&&n't Save")),
            ...
            choice = (await this.dialogService.prompt({
                ...
                detail: hasUnhandledConflicts
                    ...
                    : isMany
                        ? ( localize(7350, "The merge results will be lost if you don't save them."))
                        : ( localize(7351, "The merge result will be lost if you don't save it.")),
                buttons,
                cancelButton: {
    ...
    async save(options) {
        ...
        (async () => {
            const { confirmed } = await this.dialogService.confirm({
                message: ( localize(7352, "Do you want to accept the merge result?")),
                detail: ( localize(
                    7353,
                    "This will write the merge result to the original file and close the merge editor."
                )),
                primaryButton: ( localize(7354, '&&Accept Merge'))
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static { this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource('merge-editor.source', ( localize(7355, "Before Resolving Conflicts In Merge Editor"))); }
    async createInputModel(args) {
        const store = ( new DisposableStore());
        ...
        if (someDirty) {
            ...
            const { result } = await this._dialogService.prompt({
                ...
                detail: someUnhandledConflicts ?
                    ...
                    : isMany
                        ? ( localize(7360, "Your changes will be lost if you don't save them."))
                        : ( localize(7361, "Your changes will be lost if you don't save them.")),
                buttons: [
                    {
                        label: someUnhandledConflicts
                            ? ( localize(7362, '&&Save with Conflicts'))
                            : ( localize(7363, '&&Save')),
                        run: () => ConfirmResult.SAVE
                    },
                    {
                        label: ( localize(7364, "Do&&n't Save")),
                        run: () => ConfirmResult.DONT_SAVE
                    }
                ],
                cancelButton: {
            ...
            const { confirmed, checkboxChecked } = await this._dialogService.confirm({
                message: isMany
                    ? ( localize(7365, 'Do you want to close {0} merge editors?', inputModels.length))
                    : ( localize(
                    7366,
                ...
                detail: someUnhandledConflicts ?
                    isMany
                        ? ( localize(7367, "The files contain unhandled conflicts."))
                        : ( localize(7368, "The file contains unhandled conflicts."))
                    : undefined,
                primaryButton: someUnhandledConflicts
                    ? ( localize(7369, '&&Close with Conflicts'))
                    : ( localize(7370, '&&Close')),
                checkbox: { label: ( localize(7371, "Do not ask me again")) }
            });
            if (checkboxChecked) {
