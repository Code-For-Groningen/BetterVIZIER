...
let ViewQuickAccessProvider = class ViewQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    constructor(viewDescriptorService, viewsService, outputService, terminalService, terminalGroupService, debugService, paneCompositeService, contextKeyService) {
        super(ViewQuickAccessProvider_1.PREFIX, {
            noResultsPick: {
                label: ( localize(8388, "No matching views")),
                containerLabel: ''
            }
        });
    ...
    doGetViewPickItems() {
        ...
        addPaneComposites(ViewContainerLocation.Sidebar, ( localize(8389, "Side Bar")));
        addPaneComposites(ViewContainerLocation.Panel, ( localize(8390, "Panel")));
        addPaneComposites(ViewContainerLocation.AuxiliaryBar, ( localize(8391, "Secondary Side Bar")));
        ...
        this.terminalGroupService.groups.forEach((group, groupIndex) => {
            group.terminalInstances.forEach((terminal, terminalIndex) => {
                const label = ( localize(8392, "{0}: {1}", `${groupIndex + 1}.${terminalIndex + 1}`, terminal.title));
                viewEntries.push({
                    label,
                    containerLabel: ( localize(8393, "Terminal")),
                    accept: async () => {
        ...
        this.debugService.getModel().getSessions(true).filter(s => s.hasSeparateRepl()).forEach((session, _) => {
            ...
            viewEntries.push({
                label,
                containerLabel: ( localize(8394, "Debug Console")),
                accept: async () => {
        ...
        for (const channel of channels) {
            viewEntries.push({
                label: channel.label,
                containerLabel: ( localize(8395, "Output")),
...
class OpenViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8396, 'Open View')),
...
class QuickAccessViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8397, 'Quick Open View')),
            category: Categories.View,
            f1: false,
            keybinding: {
