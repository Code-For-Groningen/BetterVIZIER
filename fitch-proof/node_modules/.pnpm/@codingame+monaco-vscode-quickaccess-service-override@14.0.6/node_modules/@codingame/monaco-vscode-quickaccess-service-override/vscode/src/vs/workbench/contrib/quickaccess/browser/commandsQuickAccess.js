...
import { TriggerAction } from '@codingame/monaco-vscode-d0fb86d3-2a47-594e-955b-9a24631a7124-common/vscode/vs/platform/quickinput/browser/pickerQuickAccess';
...
import { ASK_QUICK_QUESTION_ACTION_ID, CHAT_OPEN_ACTION_ID } from '@codingame/monaco-vscode-ea14e352-8f1c-5569-b79a-8a96a53e8abe-common/vscode/vs/workbench/contrib/chat/browser/actions/chatConstants';
...
import { RelatedInformationType } from '@codingame/monaco-vscode-fc985c90-0334-5b62-88bc-73e2efa0b80b-common/vscode/vs/workbench/services/aiRelatedInformation/common/aiRelatedInformation';
...
import { createKeybindingCommandQuery } from '@codingame/monaco-vscode-d609a7d3-bf87-551a-884f-550a8b327ec5-common/vscode/vs/workbench/services/preferences/browser/keybindingsEditorModel';
...
let CommandsQuickAccessProvider = class CommandsQuickAccessProvider extends AbstractEditorCommandsQuickAccessProvider {
    ...
    constructor(editorService, menuService, extensionService, instantiationService, keybindingService, commandService, telemetryService, dialogService, configurationService, editorGroupService, preferencesService, productService, aiRelatedInformationService, chatAgentService) {
        super({
            ...
            noResultsPick: () => ({
                label: ( localize(8367, "No matching commands")),
                commandId: ''
            }),
    ...
    async getCommandPicks(token) {
        ...
        return ([
            ...
            buttons: [{
                    ...
                    tooltip: ( localize(8368, "Configure Keybinding")),
                }],
            trigger: () => {
    ...
    async getAdditionalCommandPicks(allPicks, picksSoFar, filter, token) {
        ...
        if (defaultAgent) {
            additionalPicks.push({
                label: ( localize(8369, "Ask {0}: {1}", defaultAgent.fullName, filter)),
    ...
    getGlobalCommandPicks() {
        ...
        for (const action of globalCommandsMenuActions) {
            ...
            if (category) {
                label = ( localize(8370, "{0}: {1}", category, label));
            }
...
class ShowAllCommandsAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8371, 'Show All Commands')),
            keybinding: {
...
class ClearCommandHistoryAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(8372, 'Clear Command History')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (commandHistoryLength > 0) {
            const { confirmed } = await dialogService.confirm({
                type: 'warning',
                message: ( localize(8373, "Do you want to clear the history of recently used commands?")),
                detail: ( localize(8374, "This action is irreversible!")),
                primaryButton: ( localize(8375, "&&Clear"))
            });
            if (!confirmed) {
                return;
            }
