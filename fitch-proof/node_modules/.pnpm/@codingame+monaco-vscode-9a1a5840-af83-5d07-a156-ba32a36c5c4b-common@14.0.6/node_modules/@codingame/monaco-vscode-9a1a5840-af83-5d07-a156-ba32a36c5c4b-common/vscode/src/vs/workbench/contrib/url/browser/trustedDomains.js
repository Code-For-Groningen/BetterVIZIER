...
const manageTrustedDomainSettingsCommand = {
    ...
    description: {
        description: ( localize2(10678, 'Manage Trusted Domains')),
        args: []
    },
...
async function configureOpenerTrustedDomainsHandler(trustedDomains, domainToConfigure, resource, quickInputService, storageService, editorService, telemetryService) {
    ...
    options.push({
        type: 'item',
        label: ( localize(10679, 'Trust {0}', domainToConfigure)),
        id: 'trust',
    ...
    if (isIP) {
        if (parsedDomainToConfigure.authority.includes(':')) {
            ...
            options.push({
                type: 'item',
                label: ( localize(10680, 'Trust {0} on all ports', base)),
                toTrust: base + ':*',
                id: 'trust'
            });
        }
        ...
        options.push({
            type: 'item',
            label: ( localize(10681, 'Trust {0} and all its subdomains', domainEnd)),
    ...
    options.push({
        type: 'item',
        label: ( localize(10682, 'Trust all domains (disables link protection)')),
        toTrust: '*',
        id: 'trust'
    });
    options.push({
        type: 'item',
        label: ( localize(10683, 'Manage Trusted Domains')),
        id: 'manage'
    });
