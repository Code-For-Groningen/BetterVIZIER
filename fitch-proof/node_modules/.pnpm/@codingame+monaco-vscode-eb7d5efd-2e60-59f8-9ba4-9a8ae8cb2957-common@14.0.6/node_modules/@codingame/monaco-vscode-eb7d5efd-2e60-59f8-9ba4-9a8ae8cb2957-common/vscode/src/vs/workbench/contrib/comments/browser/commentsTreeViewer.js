...
import { TimestampWidget } from '@codingame/monaco-vscode-9ee79c1a-3f03-568b-8eac-b02513a98b68-common/vscode/vs/workbench/contrib/comments/browser/timestamp';
import { Codicon } from '@codingame/monaco-vscode-api/vscode/vs/base/common/codicons';
import { ThemeIcon } from '@codingame/monaco-vscode-api/vscode/vs/base/common/themables';
import { commentViewThreadStateColorVar, getCommentThreadStateIconColor } from '@codingame/monaco-vscode-9ee79c1a-3f03-568b-8eac-b02513a98b68-common/vscode/vs/workbench/contrib/comments/browser/commentColors';
import { CommentThreadState, CommentThreadApplicability } from '@codingame/monaco-vscode-api/vscode/vs/editor/common/languages';
import { FilterOptions } from './commentsFilterOptions.js';
import { basename } from '@codingame/monaco-vscode-api/vscode/vs/base/common/resources';
import { openLinkFromMarkdown } from '@codingame/monaco-vscode-api/vscode/vs/editor/browser/widget/markdownRenderer/browser/markdownRenderer';
...
import { getContextMenuActions, createActionViewItem } from '@codingame/monaco-vscode-api/vscode/vs/platform/actions/browser/menuEntryActionViewItem';
...
const COMMENTS_VIEW_TITLE = ( localize2(5026, "Comments"));
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return localize(5027, "{0} replies", commentCount - 1);
        }
        else if (commentCount === 2) {
            return localize(5028, "1 reply");
        }
        else {
            return localize(5029, "1 comment");
        }
    }
    getRenderedComment(commentBody, disposables) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(5030, "Image: {0}", image.alt)) : ( localize(5031, "Image"));
    ...
    renderElement(node, index, templateData, height) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(5032, "Outdated"));
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(5033, "[Ln {0}]", node.element.range.startLineNumber));
            }
            else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(5035, "Last reply from {0}", lastComment.userName));
