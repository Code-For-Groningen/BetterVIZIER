...
import { findFirstIdxMonotonousOrArrLen } from '@codingame/monaco-vscode-api/vscode/vs/base/common/arraysFind';
...
import { ModelDecorationOptions, TextModel } from '@codingame/monaco-vscode-api/vscode/vs/editor/common/model/textModel';
...
import { COMMENTEDITOR_DECORATION_KEY } from '@codingame/monaco-vscode-9ee79c1a-3f03-568b-8eac-b02513a98b68-common/vscode/vs/workbench/contrib/comments/browser/commentReply';
    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(5005, "Line {0}", commentRangeStart))) : status(( localize(5006, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(5009, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(( localize(5010, "The cursor must be within a commenting range to add a comment.")));
                }
                else {
                    this.notificationService.error(( localize(5011, "File comments are not allowed on this file.")));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, { placeHolder: ( localize(5012, "Select Comment Provider")), matchOnDescription: true }).then(pick => {
                    if (!pick) {
                        return;
                    }
