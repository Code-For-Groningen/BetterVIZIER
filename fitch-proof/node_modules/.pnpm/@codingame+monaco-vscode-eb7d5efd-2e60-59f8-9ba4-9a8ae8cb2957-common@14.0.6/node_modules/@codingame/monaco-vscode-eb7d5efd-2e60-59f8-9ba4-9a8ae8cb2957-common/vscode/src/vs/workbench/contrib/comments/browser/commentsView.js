...
import { append, $, isActiveElement } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
...
import { CommentThreadApplicability } from '@codingame/monaco-vscode-api/vscode/vs/editor/common/languages';
...
let CommentsPanel = class CommentsPanel extends FilterViewPane {
    ...
    constructor(options, instantiationService, viewDescriptorService, editorService, configurationService, contextKeyService, contextMenuService, keybindingService, openerService, themeService, commentService, telemetryService, hoverService, uriIdentityService, storageService, pathService) {
        ...
        super({
            ...options,
            filterOptions: {
                placeholder: ( localize(5036, "Filter (e.g. text, author)")),
                ariaLabel: ( localize(5037, "Filter comments")),
    ...
    updateFilter() {
        ...
        this.filterWidget.updateBadge(total === filtered || total === 0 ? undefined : ( localize(5038, "Showing {0} of {1}", filtered, total)));
    ...
    getScreenReaderInfoForNode(element, forAriaLabel) {
        ...
        if (forAriaLabel && this.configurationService.getValue(AccessibilityVerbositySettingId.Comments)) {
            ...
            accessibleViewHint = kbLabel ? ( localize(5039, "Inspect this in the accessible view ({0}).\n", kbLabel)) : ( localize(
                5040,
    ...
    getReplyCountAsString(node, forAriaLabel) {
        return node.replies.length && !forAriaLabel ? ( localize(5046, " {0} replies,", node.replies.length)) : '';
    }
    createTree() {
            ...
            accessibilityProvider: {
                getAriaLabel: (element) => {
                    if (element instanceof CommentsModel) {
                        return localize(5047, "Comments for current workspace");
                    }
