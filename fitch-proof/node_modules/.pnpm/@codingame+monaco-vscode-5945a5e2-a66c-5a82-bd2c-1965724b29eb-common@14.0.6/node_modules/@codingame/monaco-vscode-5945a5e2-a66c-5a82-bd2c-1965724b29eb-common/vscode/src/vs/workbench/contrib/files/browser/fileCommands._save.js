...
async function doSaveEditors(accessor, editors, options) {
    ...
    try {
        ...
        if (!isCancellationError(error)) {
            const actions = [toAction({ id: 'workbench.action.files.saveEditors', label: ( localize(6666, "Retry")), run: () => instantiationService.invokeFunction(accessor => doSaveEditors(accessor, editors, options)) })];
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({ id: 'workbench.action.files.revertEditors', label: editorsToRevert.length > 1 ? ( localize(6667, "Revert All")) : ( localize(6668, "Revert")), run: () => editorService.revert(editorsToRevert) }));
            }
