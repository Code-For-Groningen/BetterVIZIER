...
import { TextFileContentProvider } from '@codingame/monaco-vscode-eea37658-2307-56a5-8ed0-b2752c7b7a50-common/vscode/vs/workbench/contrib/files/common/files';
...
let ResolveConflictLearnMoreAction = class ResolveConflictLearnMoreAction extends Action {
    constructor(openerService) {
        super('workbench.files.action.resolveConflictLearnMore', ( localize(6510, "Learn More")));
...
let DoNotShowResolveConflictLearnMoreAction = class DoNotShowResolveConflictLearnMoreAction extends Action {
    constructor(storageService) {
        super('workbench.files.action.resolveConflictLearnMoreDoNotShowAgain', ( localize(6511, "Don't Show Again")));
        this.storageService = storageService;
    }
...
let ResolveSaveConflictAction = class ResolveSaveConflictAction extends Action {
    constructor(model, editorService, notificationService, instantiationService, productService) {
        super('workbench.files.action.resolveConflict', ( localize(6512, "Compare")));
        this.model = model;
        this.editorService = editorService;
        this.notificationService = notificationService;
        this.instantiationService = instantiationService;
...
class SaveModelElevatedAction extends Action {
    constructor(model, options, triedToUnlock) {
        super('workbench.files.action.saveModelElevated', triedToUnlock ? isWindows ? ( localize(6514, "Overwrite as Admin...")) : ( localize(6515, "Overwrite as Sudo...")) : isWindows ? ( localize(6516, "Retry as Admin...")) : ( localize(6517, "Retry as Sudo...")));
        this.model = model;
...
class RetrySaveModelAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.saveModel', ( localize(6518, "Retry")));
        this.model = model;
        this.options = options;
    }
    async run() {
...
class RevertModelAction extends Action {
    constructor(model) {
        super('workbench.files.action.revertModel', ( localize(6519, "Revert")));
        this.model = model;
    }
    async run() {
        if (!this.model.isDisposed()) {
...
class UnlockModelAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.unlock', ( localize(6520, "Overwrite")));
        this.model = model;
        this.options = options;
    }
    async run() {
...
class SaveModelIgnoreModifiedSinceAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.saveIgnoreModifiedSince', ( localize(6520, "Overwrite")));
        this.model = model;
        this.options = options;
    }
    async run() {
...
let ConfigureSaveConflictAction = class ConfigureSaveConflictAction extends Action {
    constructor(preferencesService) {
        super('workbench.files.action.configureSaveConflict', ( localize(6521, "Configure")));
