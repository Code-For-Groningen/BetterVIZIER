...
import { prepareQuery, scoreItemFuzzy, compareItemsByFuzzyScore } from '@codingame/monaco-vscode-d0fb86d3-2a47-594e-955b-9a24631a7124-common/vscode/vs/base/common/fuzzyScorer';
...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    static { this.NO_RESULTS_PICK = {
        label: ( localize(8772, "No matching results"))
    }; }
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({ type: 'separator', label: ( localize(8773, "recently opened")) });
            }
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                return additionalPicks.length > 0 ? [
                    { type: 'separator', label: this.configuration.includeSymbols ? ( localize(8774, "file and symbol results")) : ( localize(8775, "file results")) },
                    ...additionalPicks
                ] : [];
            })(),
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders()
            ...
            return {
                ...
                ariaLabel: ( localize(8776, "{0}, {1}", label, helpEntry.description)),
                accept: () => {
        ...
        if (this.quickChatService.enabled) {
            providers.push({
                label: ( localize(8777, "Open Quick Chat")),
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? ( localize(8781, "{0} unsaved changes", labelAndDescription)) : labelAndDescription,
            description,
