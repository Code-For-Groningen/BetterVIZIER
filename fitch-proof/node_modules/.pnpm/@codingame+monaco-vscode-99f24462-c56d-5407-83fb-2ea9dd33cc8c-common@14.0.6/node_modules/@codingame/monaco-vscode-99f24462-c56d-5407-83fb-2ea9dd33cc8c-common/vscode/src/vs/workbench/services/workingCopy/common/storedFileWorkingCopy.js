...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: ( localize(12329, "Saving '{0}'", this.name)),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
            ...
            this.inErrorMode = false;
            progress.report({ message: ( localize(12330, "Writing into file...")) });
    ...
    doHandleSaveError(error, options) {
        ...
        if (fileOperationError.fileOperationResult === FileOperationResult.FILE_MODIFIED_SINCE) {
            ...
            primaryActions.push(toAction({ id: 'fileWorkingCopy.overwrite', label: ( localize(12332, "Overwrite")), run: () => this.save({ ...options, ignoreModifiedSince: true, reason: SaveReason.EXPLICIT }) }));
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: ( localize(12333, "Revert")), run: () => this.revert() }));
        }
        else {
            ...
            if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
                primaryActions.push(toAction({
                    ...
                    label: triedToUnlock ?
                        isWindows ? ( localize(12334, "Overwrite as Admin...")) : ( localize(12335, "Overwrite as Sudo...")) :
                        isWindows ? ( localize(12336, "Retry as Admin...")) : ( localize(12337, "Retry as Sudo...")),
                    run: () => {
                ...
                primaryActions.push(toAction({ id: 'fileWorkingCopy.unlock', label: ( localize(12332, "Overwrite")), run: () => this.save({ ...options, writeUnlock: true, reason: SaveReason.EXPLICIT }) }));
            }
            else {
                primaryActions.push(toAction({ id: 'fileWorkingCopy.retry', label: ( localize(12338, "Retry")), run: () => this.save({ ...options, reason: SaveReason.EXPLICIT }) }));
            }
            primaryActions.push(toAction({
                id: 'fileWorkingCopy.saveAs',
                label: ( localize(12339, "Save As...")),
                run: async () => {
