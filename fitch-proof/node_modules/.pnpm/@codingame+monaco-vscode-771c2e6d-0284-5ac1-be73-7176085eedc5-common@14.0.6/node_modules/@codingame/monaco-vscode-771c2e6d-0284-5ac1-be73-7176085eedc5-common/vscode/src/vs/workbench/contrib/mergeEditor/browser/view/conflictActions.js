...
import { ModifiedBaseRangeStateKind, ModifiedBaseRangeState } from '@codingame/monaco-vscode-464d4abe-0937-5367-af84-1b9279e14ea3-common/vscode/vs/workbench/contrib/mergeEditor/browser/model/modifiedBaseRange';
    ...
    constructor(viewModel, modifiedBaseRange) {
        ...
        this.resultItems = derived(this, reader => {
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push({
                    text: ( localize(7387, "Manual Resolution")),
                    tooltip: ( localize(7388, "This conflict has been resolved manually.")),
                });
            }
            else if (state.kind === ModifiedBaseRangeStateKind.base) {
                result.push({
                    text: ( localize(7389, 'No Changes Accepted')),
                    tooltip: ( localize(
                        7390,
            ...
            const stateToggles = [];
            if (state.includesInput1) {
                stateToggles.push(command(( localize(7391, 'Remove {0}', model.input1.title)), async () => {
                    transaction((tx) => {
                ...
                }, ( localize(7392, 'Remove {0} from the result document.', model.input1.title))));
            }
            if (state.includesInput2) {
                stateToggles.push(command(( localize(7391, 'Remove {0}', model.input2.title)), async () => {
                    transaction((tx) => {
                ...
                }, ( localize(7392, 'Remove {0} from the result document.', model.input2.title))));
            }
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push(command(( localize(7393, 'Reset to base')), async () => {
                    transaction((tx) => {
        ...
        return derived(reader => {
            ...
            if (state.kind !== ModifiedBaseRangeStateKind.unrecognized && !state.isInputIncluded(inputNumber)) {
                if (!state.isInputIncluded(otherInputNumber) || !this.viewModel.shouldUseAppendInsteadOfAccept.read(reader)) {
                    result.push(command(( localize(7395, "Accept {0}", inputData.title)), async () => {
                        transaction((tx) => {
                    ...
                    }, ( localize(7396, "Accept {0} in the result document.", inputData.title))));
                    if (modifiedBaseRange.canBeCombined) {
                        const commandName = modifiedBaseRange.isOrderRelevant
                            ? ( localize(7397, "Accept Combination ({0} First)", inputData.title))
                            : ( localize(7398, "Accept Combination"));
                    ...
                    result.push(command(( localize(7400, "Append {0}", inputData.title)), async () => {
                        transaction((tx) => {
                            model.setState(modifiedBaseRange, state.withInputValue(inputNumber, true, false), inputNumber, tx);
                            model.telemetry.reportAcceptInvoked(inputNumber, state.includesInput(otherInputNumber));
                        });
                    }, ( localize(7401, "Append {0} to the result document.", inputData.title))));
                    if (modifiedBaseRange.canBeCombined) {
                        result.push(command(( localize(7402, "Accept Combination", inputData.title)), async () => {
                            transaction((tx) => {
                ...
                if (!model.isInputHandled(modifiedBaseRange, inputNumber).read(reader)) {
                    result.push(command(( localize(7403, 'Ignore')), async () => {
                        transaction((tx) => {
                            model.setInputHandled(modifiedBaseRange, inputNumber, true, tx);
                        });
                    }, ( localize(7404, "Don't take this side of the conflict."))));
                }
            }
            return result;
        });
